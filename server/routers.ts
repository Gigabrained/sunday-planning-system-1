...
export const appRouter = router({
        ...
        try {
          ...
          if (input.agencyName) {
            ...
            if (agency && agency.slackChannelId) {
              try {
                await postContentApprovalNotification(
                  agency.slackChannelId,
                  input.agencyName,
                  input.taskName || 'Content Review',
                  input.contentLink
                );
                console.log(`[Dashboard] Sent approval notification to ${input.agencyName} (${agency.slackChannelId})`);
              } catch (slackError) {
  ...
  // Job Title Mappings (for split testing)
  jobTitleMappings: router({
    list: protectedProcedure.query(async () => {
      return await db.getAllJobTitleMappings();
    }),
    
    listByRole: protectedProcedure
      .input(z.object({ roleId: z.number() }))
      .query(async ({ input }) => {
        return await db.getJobTitleMappingsByRole(input.roleId);
      }),
    
    create: protectedProcedure
      .input(z.object({
        roleId: z.number(),
        workableJobTitle: z.string().min(1).max(255),
      }))
      .mutation(async ({ input }) => {
        await db.createJobTitleMapping(input);
        return { success: true };
      }),
    ...
    delete: protectedProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ input }) => {
        await db.deleteJobTitleMapping(input.id);
        return { success: true };
      }),
  }),
  ...
  jobPostings: router({
    ...
    listActive: protectedProcedure.query(async () => {
      return await db.getActiveJobPostings();
    }),
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        dailySpend: z.number().int().positive(),
        startDate: z.string(),
        status: z.enum(['active', 'paused', 'closed']).default('active'),
        totalApplicants: z.number().int().min(0).default(0),
        notes: z.string().optional(),
      }))
      .mutation(async ({ input }) => {
        await db.createJobPosting({
          ...input,
    ...
    update: protectedProcedure
      .input(z.object({
        ...
        dailySpend: z.number().int().positive().optional(),
        startDate: z.string().optional(),
        status: z.enum(['active', 'paused', 'closed']).optional(),
        totalApplicants: z.number().int().min(0).optional(),
        notes: z.string().optional(),
      }))
      .mutation(async ({ input }) => {
        ...
        await db.updateJobPosting(id, updateData);
        return { success: true };
      }),
  ...
  invoices: router({ 
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        dailyFees: z.number().int(),
      }))
      .mutation(async ({ input }) => {
        await db.createInvoice({
          ...input,
          transactionDate: new Date(input.transactionDate),
          adStartDate: new Date(input.adStartDate),
          adEndDate: new Date(input.adEndDate),
        });
        return { success: true };
      }),
  ...
  // Analytics
  analytics: router({
    spendingByRole: protectedProcedure.query(async () => {
      return await db.getSpendingByRole();
    }),
    
    spendingByLocation: protectedProcedure.query(async () => {
      return await db.getSpendingByLocation();
    }),
    
    totalStats: protectedProcedure.query(async () => {
      const stats = await db.getTotalStats();
      return {
        ...stats,
        avgCostPerApplicant: stats.totalApplicants > 0 
          ? stats.totalSpending / stats.totalApplicants 
          : null,
      };
    }),
  }),

  // Recruiters (top-level for compatibility)
  recruiters: router({
    list: protectedProcedure.query(async () => {
      return await db.getAllRecruiters();
    }),
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        slackChannelId: z.string().min(1).max(255),
      }))
      .mutation(async ({ input }) => {
        const id = await db.createRecruiter(input);
        const recruiter = await db.getRecruiterById(id);
        return { success: true, id, recruiterCode: recruiter?.recruiterCode || '' };
      }),
    ...
    update: protectedProcedure
      .input(z.object({
        ...
        recruiterCode: z.string().min(1).max(50).optional(),
        slackChannelId: z.string().min(1).max(255).optional(),
  ...
  // Job Assignments (top-level for compatibility)
  jobAssignments: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        const recruiter = await db.getRecruiterById(input.recruiterId);
        if (!recruiter) {
          throw new TRPCError({ code: 'NOT_FOUND', message: 'Recruiter not found' });
        }
        
        const nomenclature = `${input.jobTitle} - ${recruiter.recruiterCode}`;
  ...
  // Hiring Priorities (top-level for compatibility)
  priorities: router({
    list: protectedProcedure.query(async () => {
      return await db.getAllHiringPriorities();
    }),
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        await db.createHiringPriority(input);
        return { success: true };
      }),
  ...
  // Workable Integration (top-level for compatibility)
  workable: router({
    getAllJobs: protectedProcedure.query(async () => {
      const { getWorkableJobs } = await import('./workable');
